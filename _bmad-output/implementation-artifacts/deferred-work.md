...
## Deferred from: bmad-build implementation of Story 1 — Local Private Server (2026-08-17)

- source_spec: `_bmad-output/specs/spec-screeps_ai/stories/1-local-private-server.md`
  summary: Story 1's committed implementation (`scripts/server-bootstrap.js`, `scripts/server-reset.js`, `screeps-launcher/config.yml.example`, README's Local Server section) targets `screeps-launcher` (native Go binary), but the operator's actual working local server uses a different tool — `jomik/screeps-server` via Docker Compose, with `screepsmod-konami` added to relax requirements — after `screeps-launcher` hit a chain of blockers live (missing `g++`, an `isolated-vm` GCC-15 incompatibility needing a hand-patch, then a Docker-mount stray-directory issue, then screeps-launcher's own bundled dependency-resolution pins being stale against current mod versions). The repo, SPEC.md, `stack.md`, `deferred.md`, and `ARCHITECTURE_SPINE.md` AD-14 all still describe `screeps-launcher` as the decided tool; none of that has been reworked to match what's actually running.
  evidence: Operator confirmed live, 2026-08-17: "I got something working. I used a different launcher jomik/screeps-server which allows me to add mods and update the config on the fly... Things working as before." Explicitly deferred rather than reworked now — operator chose "leave it for now, just get back to other work" when asked whether to reconcile the story/spec to the new tool.
